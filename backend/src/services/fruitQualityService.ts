...
// Full interface with all 7 fields lives in frontend/src/types.ts.
// Backend uses a compatible subset (only fields the controller needs to pass through).
...
// NOTE: Intentionally NOT a module-level constant — reads env var dynamically
// so that Jest tests can override process.env per-test in beforeEach/afterEach.
...
export const fruitQualityService = {
   ...
   * Never throws — designed to be called defensively.
   */
   ...
   *
   * THROWS FruitClassificationError on any failure — no silent fake fallback.
   * A wrong quality grade is worse than a clear "service unavailable" message.
   */
  async classifyFruitImage(
    ...
    try {
      ...
      // We use a Blob to pass the buffer with the correct mimetype
